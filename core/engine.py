...
class RLMEngine:
    ...
    def __init__(self, api_key, vault_path, cache_path):
        ...
        self.root_model = os.getenv("ROOT_MODEL", "claude-3-haiku-20240307")
        self.sub_model = os.getenv("SUB_MODEL", "claude-3-haiku-20240307")
    ...
    def debug(self, log_content):
        ...
        # Extract the first ERROR or FATAL line to search for (ignoring timestamp)
        query = log_content[:200]
        for line in log_content.splitlines():
            if "ERROR" in line or "FATAL" in line or "Panic" in line:
                # Strip timestamp (simple heuristic: take last 70% of line or split by spaces)
                parts = line.split(" ", 2)
                if len(parts) > 2:
                    query = parts[2]  # Skip timestamp/level
                else:
                    query = line
                break
        
        tier1_hits = self.reader.grep(query)
        if tier1_hits:
            context = "\n".join(
    ...
    def _recursive_search(self, log_content, depth, trace):
        ...
        for _step in range(15):  # max 15 code-execute steps per depth level
            ...
            try:
                ...
                if res_json is None:
                    # Retry logic for Haiku robustness
                    messages.append({"role": "assistant", "content": raw_text})
                    messages.append({"role": "user", "content": "Error: Invalid JSON format. Please output valid JSON only."})
                    continue

            action = res_json.get("action", "")
            if action == "SEARCH":  # Alias for Haiku hallucination
                action = "EXECUTE_CODE"
        ...
        return {"status": "STEP_LIMIT", "msg": f"Exhausted 15 steps at depth {depth}."}
    ...
    def _synthesize_from_context(self, error, vault_context):
        ...
        # Fallback: If JSON failed or is missing 'solution', use the vault context directly
        # This guarantees we don't drop a valid Tier 1 match just because Haiku has bad output formatting
        if res_json is None or (isinstance(res_json, dict) and not res_json.get("report", {}).get("solution") and not res_json.get("solution")):
             return {
                "status": "RESOLVED_BY_VAULT",
                "report": {
                    "root_cause": "Legacy Issue (Found in Vault)",
                    "solution": f"# Solution extracted from Vault History:\n\n{vault_context}",
                    "hypothesis": "Exact match in historical records.",
                }
            }
    ...
    def _extract_json(text):
        ...
        # Try to find bare JSON object (greedy - from first { to last })
        ...
        # Try non-greedy match (find first valid JSON object)
        match = re.search(r"\{.*?\}", text, re.DOTALL)
        if match:
            try:
                return json.loads(match.group(0))
